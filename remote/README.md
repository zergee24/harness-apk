...
For legacy `turn.start`, `CommandID` (or `RequestID` when no command ID exists) is the durable idempotency boundary and is also sent as `clientUserMessageId`. A persisted thread can be listed or read without being loaded into the current app-server process. If the first `turn/start` is definitively rejected with `thread not found`, Bridge calls `thread/resume` for the same thread ID and safely retries `turn/start` once with the same client message identity. It never creates a replacement thread for this flow. If Codex app-server succeeds but TurnID parsing or route persistence fails, Bridge returns `UNKNOWN/RECONCILING` with `retrySafe=false`, saves the app-server result when available, and retries only the local route update. It does not call `turn/start` a second time for an ambiguous outcome.
