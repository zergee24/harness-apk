...
- GitHub Actions 的 `test` / `prod` 通道在未显式传 `version_code` 时，默认用 `通道基础 versionCode + GitHub run number` 生成构建号。通道基础值优先读取仓库变量 `TEST_VERSION_CODE_BASE` / `PROD_VERSION_CODE_BASE`，未配置时回退到 Gradle 基础值；如果本地真机验收包使用了更高构建号，发布前必须上调对应仓库变量，确保新的远端构建号仍然严格递增。
