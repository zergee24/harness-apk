...
0. **扫描版 PDF 转图走视觉管道**（2026-09-04 增补，用户拍板）：PDF 抽不出有效文本层（<50 字符）时判定为扫描件，`PdfRenderer` 逐页渲染 JPEG（≤单消息图片上限 4 张，对齐「拍照提问」），交给视觉模型读图。**不接 OCR**——视觉模型对版式化文档的理解优于 OCR 文本，且不扩大数据面；OCR 仅在「无视觉模型/超长扫描件」场景再评估（阿里云 OCR 凭证可复用配置包管道）。
