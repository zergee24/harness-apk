...
- 语音方案：保留系统语音，新增 OpenAI-compatible `/audio/transcriptions` API 转写；复用“模型配置”中加密保存的 Provider 与 API Key，只持久化 Provider ID 和转写模型。录音暂存在 App 私有缓存，转写结束、失败或取消后删除，不自动发送。
...
- 待复验：需要在荣耀真机配置一个支持 `/audio/transcriptions` 的 Provider 后执行 A2；模拟器仅验证协议与状态机，不替代真实 API 音频转写。
...
- 系统模式优先使用 `SpeechRecognizer`，不可用时降级 `RecognizerIntent`；荣耀等无可用系统服务设备可显式选择 API 转写。
- API 转写复用已有加密 Provider，临时 M4A 在转写结束、失败或取消后删除；不保存原始音频，不自动发送。
...
界面保留：系统/API 转写方式、转写语言、API 模式已有 Provider 与模型、回复朗读开关与语速、麦克风权限状态。旧字段继续兼容读取，但自动填入、自动发送、保存音频不作为主链路前置条件。
