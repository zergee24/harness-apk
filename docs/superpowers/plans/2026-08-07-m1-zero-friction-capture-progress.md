...
### 2026-08-08 G1 真机验收修复

- 用户反馈：A1、A3 通过；A2 失败，内嵌系统识别服务提示“系统语音识别暂时不可用”。
- 根因：设备已注册 `SpeechRecognizer`，但运行时返回网络、网络超时或服务端错误；原实现只在静态能力检查失败时降级，没有覆盖“服务存在但本轮不可用”。
- 修复：尚未返回 partial 的内嵌识别遇到上述错误时，自动降级到可解析的系统 `RecognizerIntent`；权限错误、已有 partial、无系统识别 Activity、用户取消或页面离开均不触发降级。
- 自动化：API 36 模拟器运行语音适配、单一主按钮与草稿持久化定向测试 6/6 成功；模拟器同时确认 `RecognitionService` 与 `RECOGNIZE_SPEECH` Activity 可解析。
- 待复验：安装最新 `test` 包后重新执行 A2，确认系统识别界面返回的中文可编辑、不会自动发送，手动发送后只有修改后的单条消息。

### 2026-08-08 G1 荣耀设备与输入区二次修复

- 用户反馈：荣耀设备仍无法使用 Google/系统语音；同时要求恢复外侧“加号/发送”同位切换，把语音收进输入框，并精简上下文摘要。
- 语音方案：保留系统语音，新增 OpenAI-compatible `/audio/transcriptions` API 转写；该阶段先以已有模型 Provider 验证协议与录音链路，录音暂存在 App 私有缓存，转写结束、失败或取消后删除，不自动发送。
- 输入交互：空输入时外侧主按钮为加号，有文字或图片时同位切换为发送，生成中切换为停止；输入框内保留独立的小型语音开始/停止按钮。
- 上下文摘要：主行只展示模型和推理强度；项目、非默认智能体、Wiki 数量和联网状态仅在有意义时显示；上下文占用默认隐藏并保留在展开面板。
- 自动化：API multipart 协议、鉴权脱敏、私有录音清理、设置持久化、输入按钮状态与上下文摘要均已补测试；API 36 模拟器定向测试 9/9 成功，320dp/字体 1.15 下加号与发送坐标一致。
- 待复验：需要在荣耀真机配置第三方转写 Key 后执行 A2；模拟器仅验证协议与状态机，不替代真实 API 音频转写。

### 2026-08-08 G1 硅基流动产品化接入

- 决策：不接荣耀专有 Magic Live SDK；API 转写固定接入硅基流动，避免用户先创建模型 Provider 或处理厂商 SDK、签名和机型兼容。
- 配置：语音能力页提供“系统语音 / 硅基流动”；默认选中 `FunAudioLLM/SenseVoiceSmall`，可切换 `TeleAI/TeleSpeechASR`，用户只需配置硅基流动 API Key。
- 安全：API Key 使用独立 Android Keystore 别名加密，设置页不回显明文；录音继续使用 App 私有临时文件并在成功、失败或取消后删除。
- 兼容：旧的 `CLOUD` 语音选择自动迁移为硅基流动；系统语音仍可手动切回。硅基流动按服务端自动识别语种，不展示无效的语言配置。
- 错误：401/403 引导重新配置 Key，429 提示稍后重试，503/504 提示服务繁忙；错误信息不包含 Key 或服务端响应正文。
- 自动化：默认模型、旧配置迁移、官方端点、multipart 上传、错误脱敏、真实 Android Keystore 存取/清除和设置持久化均有测试。
- 在线验证：使用与 App 相同的 M4A、`audio/mp4` multipart 请求调用硅基流动官方端点，默认 `FunAudioLLM/SenseVoiceSmall` 返回 HTTP 200 且中文转写正确；测试 Key 未写入仓库或测试资产。
- 待复验：用户配置真实 Key 后，在荣耀真机完成“说话 -> 停止 -> 文字进入输入框 -> 修改后发送”的 A2 链路。
...
- 系统模式优先使用 `SpeechRecognizer`，不可用时降级 `RecognizerIntent`；荣耀等无可用系统服务设备可显式选择硅基流动。
- 硅基流动使用独立加密 API Key，临时 M4A 在转写结束、失败或取消后删除；不保存原始音频，不自动发送。
...
| 状态 | 按钮 | 行为 |
| --- | --- | --- |
| 空文本、无附件、空闲 | 加号 | 打开拍照/相册入口 |
| 有文本或附件、空闲 | 发送 | 沿用现有发送入口 |
| 正在生成 | 停止 | 沿用现有停止生成 |

输入框内提供独立小型语音按钮，识别或录音时原位切换为停止语音；同时删除上方 Chip 行中的第二个语音入口。`ChatScreen` 只消费状态和回调，不能持有语音实现生命周期。
...
界面保留：系统/硅基流动转写方式、硅基流动 Key 与模型、系统转写语言、回复朗读开关与语速、麦克风权限状态。旧字段继续兼容读取，但自动填入、自动发送、保存音频不作为主链路前置条件。
...
- 主摘要固定展示模型和推理强度；项目、非默认 Agent、Wiki 数量与联网状态仅在有意义时展示，上下文占用和文件变更只放二级配置。
- 删除当前横向滚动的常驻 Chip 行。
...
Batch 3 退出条件：输入区不再横向滚动；一眼可读模型与推理强度，有意义的附加上下文可见；切换项目不篡改旧会话归属。
