...
输入框内语音按钮在识别或录音时切换为停止；语音不占用外侧加号/发送按钮的位置，也不放在上方配置 Chip 行。
...
```text
┌──────────────────────────────────────┐
│ GPT-5 · 超高                       ▾ │  40dp 视觉高度，48dp 点击区
│ CRM · 李德胜 · Wiki 2                │  仅有非默认上下文时出现
│ [图片预览，仅有附件时出现]           │
│ 说点什么...                    [🎙] │
└──────────────────────────────────────┘
```
...
- 删除当前可横向滚动的多个 Chip 行。
- 主摘要只固定展示模型和推理强度；未配置模型时显示“未配置模型”。
- 次摘要只在有意义时展示项目、非“普通助手”的 Agent、已启用 Wiki 数量和联网状态；空值、默认身份和“无 Wiki”不占位置。
- 文本一行省略，不能推动主按钮或改变输入区宽度。
- 上下文占用和文件变更模式默认隐藏，只在展开面板中提供；联网开启时可在次摘要提示，不再各占常驻 Chip。
...
- 没有新增顶层模式、收件箱、独立语音凭据仓储或通用附件 RAG。
