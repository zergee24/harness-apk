...
> **2026-09-04 修订：** ①试点更名——本文档 P0-A/P0-B 与《交付物层与入口收敛设计》已完成并上线的 P0-A/P0-B 重名，为消歧改为 Spike-画布 / Spike-录音（下同）。②首发设备扩展为 HiBreak 电纸书（自带手写笔）。③门禁分设备记账：输入预览 p95 < 50ms 对 Y900 保留为一票否决；HiBreak（e-ink）单独记录实测延迟分布，不套用 50ms 一票否决，由数据决定电纸书画布策略（快速局部刷新层 / 笔迹预测 / 接受更高延迟 / 降级截图批注）。其余退出条件不变。
