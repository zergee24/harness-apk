...
人物身份在新会话输入区选择，发送第一条消息后固定。智能体包的安装、更新和停用位于“设置 -> 智能体包”：导入桌面生成的 `.hbundle` 后，核对名称、版本、资料与发布者指纹再安装；状态显示“可用”时点击“开始对话”，会进入普通统一会话，不会自动创建项目。项目只提供可选的 Markdown、文件和 Git 上下文，不是智能体容器；无项目的人格会话同样合法。Agent 会话显示“基于资料模拟”，只注入本地资料检索结果，不启用联网搜索补写。
